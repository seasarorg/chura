 ...
 * 
 */
public class SrcAttrColumn implements ColumnDescriptor {

    private static final String NAME = ClassUtil
            .getShortClassName(SrcAttrColumn.class);

    private CellEditor editor;

    public SrcAttrColumn(Table table) {
        this.editor = new TextCellEditor(table);
        TableColumn column = new TableColumn(table, SWT.NONE);
        column.setText(Labels.COLUMN_COMPONENT_ATTR);
        column.setWidth(150);
    ...
    public String getText(Object element) {
        if (element instanceof MxBindingMappingRow) {
            MxBindingMappingRow row = (MxBindingMappingRow) element;
            return row.getSrcAttr();
        }
        return null;
    }
