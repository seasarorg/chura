 ...
 * 
 */
public class SqlTypeColumn implements ColumnDescriptor {

    private static final String NAME = ClassUtil
            .getShortClassName(SqlTypeColumn.class);

    private CellEditor editor;

    public SqlTypeColumn(Table table) {
        this.editor = new TextCellEditor(table);
        TableColumn column = new TableColumn(table, SWT.NONE);
        column.setText(Labels.COLUMN_SQL_TYPE);
        column.setWidth(100);
    }
    ...
    public Image getImage(Object element) {
        if (element instanceof EntityMappingRow) {
            EntityMappingRow row = (EntityMappingRow) element;
            return row.isPrimaryKey() ? Images.PRIMARY_KEY : Images.COLUMNS;
        }
