...
public class CustomizerModel {
    ...
    public static final String ASPECT_OPEN = NL
            ...
            + NL + "          <initMethod name=\"addInterceptorName\">" + NL
            + "            <arg>\"";

    public static final String ASPECT_CLOSE = "\"</arg>"
            + NL
            + "          </initMethod>"
            + NL
    ...
    public CustomizerModel(String name, String aspect) {
        if ("AspectCustomizer".equals(name) && aspect != null) {
            this.arg = new StringBuilder().append(ASPECT_OPEN).append(aspect)
                    .append(ASPECT_CLOSE).toString();
        } else {
            this.arg = name;
        }
    }
